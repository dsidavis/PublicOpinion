...
media stories shifts from pro to anti, etc. To conduct a very basic
change point detection, you first need to create a time-series of the
data, aggregated by the interval we are interested in, e.g. monthly
averages. Assuming we want the average tone across all frames, we set
\code{sepFrames = FALSE}:

<<>>=
gc.monthly = aggregateArticles(gc.articles, "monthly", sepFrames = FALSE)
@ 

The \code{aggregateArticles()} function outputs a new dataframe, with
...
<<>>=
cp = getChangePoints(tt, changeValue=0.5, var = "avgPro")
@
...
<<>>=
plot_frames(df = gc.articles, interval = "month",
            main = "Gun Control", events = cp.dates,
            eventHover = cp.dates)
